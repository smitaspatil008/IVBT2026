  ...
  return (
    <div className="mx-auto max-w-5xl py-8">
      ...
      <motion.div
        ...
        className="mb-10 overflow-hidden rounded-xl border border-[#1e1b4b]/30 bg-[#12102a] shadow-lg"
      >
        <div className="border-b border-[#1e1b4b]/20 px-5 py-3">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-[#1e1b4b]/20 bg-indigo-950/50">
              ...
              {overallStandings.map((row, index) => {
                ...
                return (
                  <motion.tr
                    ...
                    className={`border-b border-[#1e1b4b]/10 last:border-0 ${
                      style ? style.bg : ''
                    }`}
                  >
                    ...
                    <td className="px-4 py-3 text-center font-bold text-orange-500">{row.points}</td>
                  </motion.tr>
                );
              })}
            </tbody>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {groupStandings.map(({ group, standings }) => (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="overflow-hidden rounded-xl border border-[#1e1b4b]/30 bg-[#12102a] shadow-sm"
          >
            <div className="border-b border-[#1e1b4b]/20 px-4 py-2.5">
              <h3 className="text-sm font-bold text-indigo-200">{group.name}</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-[#1e1b4b]/20">
                      ...
                      >
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-2">
                            ...
                            {idx < 2 && (
                              <span className="rounded bg-orange-500/10 px-1 py-0.5 text-[10px] font-bold text-orange-500">
                                Q
                              </span>
                            )}
                          </div>
                        ...
                        <td className="px-2 py-2 text-center font-bold text-orange-500">{s.points}</td>
                      </tr>
                    );
                  })}
                </tbody>
